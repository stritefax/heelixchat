...
use crate::repository::activity_log_repository::{get_latest_activity_log_item_with_same_window, get_latest_activity_log_item, get_empty_activity_item};
...
pub async fn start_a_monitoring_cycle(handle: AppHandle, app_data_dir: &str) -> ActivityItem {
    ...
    let is_app_allowed = handle.db(|db|
        get_permission_by_app_name(&db, &active_window.app_name).unwrap()).allow;
    if !is_app_allowed {
        return get_empty_activity_item();
    }
    let (element_tree_dump, detected_actions): (String, String) = get_element_tree_by_window_app_name(&active_window.process_id.to_string());
    ...
    if ((active_window.app_name == "Safari" || active_window.app_name == "Google Chrome") && active_window.title.is_empty())
    ...
    let mut ocr_text = text_recognition_engine::get_text_from_image(
        Path::new(format!("{}.png",screenshot_path.clone().join(timestamp.format("%Y-%m-%d_%H-%M-%S").to_string()).display()).as_str())
    );

    let activity_log_item = handle.db(|database| {
   ...
   // }).unwrap();
    ...
    let recent_activity_item_option = handle.db(|database| {
        get_latest_activity_log_item_with_same_window(database, &active_window.title, &active_window.app_name)
    }).unwrap();
    ...
    if element_tree_dump.chars().take(300).collect::<String>().contains("Private Browsing")
        && (active_window.app_name == "Google Chrome" || active_window.app_name == "Safari" || active_window.app_name == "Chrome" || active_window.app_name == "Firefox") {
        return get_empty_activity_item();
    }
